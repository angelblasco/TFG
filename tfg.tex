...
\begin{teorema}
Sea $\rho: G \rightarrow GL(V)$ una representaci'on lineal de $G$ en $V$ y sea $W$ un subespacio vectorial de $V$ estable por la acci'on de $G$. Existe un subespacio complementario $W^{0}$ de $W$ en $V$ el cual es estable por la acci'on de $G$.
\end{teorema}
\begin{proof}
Sea $W'$ un subespacio complementario del subespacio $W$ de $V$ y sea $p$ la proyecci'on de $V$ sobre $W$. Definimos $p^{0}$ como \emph{el promedio} de los conjugados de $p$ por los elementos de $G$:
\[
p^{0}=\frac{1}{|G|}\sum_{t \in G} \rho (t)\cdot p \cdot \rho(t)^{-1} 
\]
ya que $p$ envia $V$ dentro de $W$ y que $\rho (t)$ conserva $W$, vemos que $p^{0}$ envia $V$ en $W$; por lo tanto $\rho (t)^{-1}x \in W$ para $x \in W$, de donde
\[
p \cdot \rho(t)^{-1}x=\rho(t)^{-1}x
\]
\[
\rho (t)\cdot p \cdot\rho(t)^{-1}x =x
\]
y
\[
p^{0}x=x
\]
Asi que $p^{0}$ es una proyecci'on de $V$ en $W$ correspondiente a alg'un complementario $W^{0}$ de $W$. Tenemos ademas
\[
\rho (s)\cdot p^{0}=p^{0}\cdot\rho (s)
\]
para todo $s \in G$. De hecho, calculando $\rho (s)\cdot p^{0} \cdot\rho (s)^{-1}$ tenemos
\[
\rho (s)\cdot p^{0}\cdot \rho (s)^{-1} = \frac{1}{|G|}\sum_{t \in G}\rho (s)\rho (t) \cdot p \cdot \rho (t)^{-1} \rho (s)^{-1} = \frac{1}{|G|}\sum_{t \in G}\rho (st)\cdot p \cdot \rho (st)^{-1} = p^{0}
\]
Si ahora $x \in W^{0}$ y $s \in G$ tenemos $p^{0}x=0$, asi que $p^{0}\cdot \rho (s)x=\rho (s)\cdot p^{0}x=0$, por lo que $\rho (s)x \in W^{0}$, lo cual muestra que $W^{0}$ es estable por $G$. Esto completa la demostraci'on.
\end{proof}\\
\\
Supongamos que $V$ esta dotado de un producto escalar $\langle x,y \rangle$ satisfaciendo las condiciones de linealidad en $x$, bilinealidad en $y$, y $\langle x,x \rangle > 0$ si $x \neq 0$. Supongamos que este producto escalar permanece invariante por la acci'on de $G$, $\langle \rho(s)x,\rho (s)y \rangle =\langle x,y \rangle$; podemos reemplazar $\langle x,y \rangle$ por $\sum_{t \in G} \langle \rho(t)x,\rho (t)y \rangle$. Bajo esta hip'otesis el complementario ortogonal $W^{0}$ de $W$ en $V$ es un subespacio complementario de $W$ estable por $G$; y asi, obtenemos una demostraci'on alternativa al anterior teorema (2.4.1). Observemos el hecho de que como el producto escalar $\langle x,y \rangle$ es invariante, implica que, si $(e_{i})$ es una base ortonormal de $V$, la matriz $\rho (s)$ con respecto a esa base es la matriz unitaria.\\
\\
Manteniendo las hip'otesis y notaci'on del teorema 2.4.1, sea $x \in V$, y sean $w$ y $w^{0}$ proyecciones en $W$ y $W^{0}$ respectivamente. Tenemos $x=w+w^{0}$, de donde $\rho (s)x=\rho (s)w+\rho (s)w^{0}$, por lo que $w$ y $w^{0}$ son estables por $G$, tenemos que $\rho (s)w \in W$ y $\rho (s)w^{0} \in W^{0}$; asi que $\rho (s)w$ y $\rho (s)w^{0}$ son proyecciones de $\rho (s)x$.\\
\\
Se sigue, entonces, que las representaciones de $W$ y $W^{0}$ determinan la representaci'on de $V$. Decimos que $V$ es suma directa de $W$ y $W^{0}$, y lo escribimos como $V=W \oplus W^{0}$. Podemos definir los elementos de $V$ como un par $(w,w^{0})$ con $w \in W$ y $w^{0} \in W^{0}$. Si $W$ y $W^{0}$ son dados en forma matricial por $R_{s}$ y $R_{s}^{0}$, $W \oplus W^{0}$ estar'a dado en forma matricial como:
\[
\left(
\begin{array}{cc}
R_{s} & 0 \\
